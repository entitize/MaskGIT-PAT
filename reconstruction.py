...
def reconstruct(I_gt):
    ...
    # Free parameters
    N_transducer   = 64   #512 in real experimental setting
    dt        = 0.5e-6  #0.025e-6 np.transpose(A)@x_gtin real experimental setting
    ...
    A_tensor = torch.from_numpy(np.transpose(A)).cuda().float()
